...
async function getUserInput() {
  const userInput = await inquirer.prompt([
    ...
    },
    {
        ...
        validate: function (input) {
          // Validate input as either a valid color keyword or a valid hexadecimal number, if time allows add more colors
          return /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$|^(red|blue|green|yellow|...)$/i.test(input) || 'Invalid color input.';
        },
      },
      {
        type: 'input',
        name: 'backgroundColor', // New input for background color, if time allows add more colors
    ...
    const backgroundColor = userInput.backgroundColor; 
